...
async def load_airports(airports):
    try:
        ...
        log.info("Airport database loaded: %d airports", len(result))
    except Exception as e:
        log.error("Failed to load airport database: %s", e)


async def load_aircraft_db(aircraft_db):
    try:
...
def decode_airline(callsign, airline_db):
...
def parse_v2_aircraft(ac_list, aircraft_db, airline_db):
    ...
    for a in ac_list:
        ...
        airline_name, flight_id = decode_airline(callsign, airline_db)
        if airline_name:
    ...
    async with httpx.AsyncClient(timeout=config.timeout) as client:
        while True:
            ...
            try:
                ...
                data = resp.json()
                aircraft = parse_v2_aircraft(data.get("ac") or [], aircraft_db, airline_db)
...
# Needed for refresh_aircraft_db
import asyncio
